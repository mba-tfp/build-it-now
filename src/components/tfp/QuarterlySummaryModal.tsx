...
import { buildQuarterlySummary } from "@/lib/tfp/exports";
import { Copy, Check, X, FileText, Printer } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";

export function QuarterlySummaryModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function copy() {
    try {
      ...
      setCopied(true);
      toast.success("Quarterly summary copied");
      ...
      <div className="relative flex max-h-[86vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-border bg-surface shadow-2xl">
        <header className="flex items-start gap-3 border-b border-border p-5">
          ...
          <div className="flex-1">
            <p className="text-[11px] uppercase tracking-wider text-muted-foreground">Quarterly summary</p>
            <h2 className="mt-0.5 font-display text-lg">Stakeholder-ready quarterly digest</h2>
          </div>
          <button
            onClick={copy}
            className={cn(
              "inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition",
              copied ? "bg-[var(--color-status-proceed)]/15 text-[var(--color-status-proceed)]" : "bg-primary text-primary-foreground hover:bg-primary/90",
            )}
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md border border-input bg-surface px-3 py-1.5 text-xs font-medium hover:bg-accent"
          >
