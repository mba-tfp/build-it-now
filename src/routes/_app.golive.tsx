    ...
    <div>
      ...
      <ScrollTable className="border border-border bg-surface/40 p-3" maxHeight="calc(100vh - 360px)">
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          return (
            <div key={g.id} className={cn("tfp-card p-5", g.war_room && "border-destructive/50 ring-2 ring-destructive/10")}>
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-1.5">
                  <span className={cn(
                    ...
                    g.status === "Live" ? "bg-[var(--color-status-proceed)]/10 text-[var(--color-status-proceed)]" :
                    g.status === "Rolled Back" ? "bg-destructive/10 text-destructive" :
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      g.war_room ? "border-destructive bg-destructive/10 text-destructive" : "border-input bg-surface text-muted-foreground hover:border-destructive/40",
                    )}
                  >
              ...
              <div className="mt-4 flex items-center gap-2">
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-muted">
                  <div className="h-full bg-primary transition-all" style={{ width: `${(doneCount / CRITERIA.length) * 100}%` }} />
                </div>
                <span className="font-mono text-xs text-muted-foreground">{doneCount}/{CRITERIA.length}</span>
              </div>
                    ...
                    <li key={c} className="flex items-start gap-2.5 rounded-md border border-border bg-surface-2 p-2.5">
                      ...
                      <div className="flex-1">
                        ...
                        {checker && cr.checked_at && (
                          <p className="mt-0.5 text-[10px] text-muted-foreground">✓ {checker.name} · {fmtDateTime(cr.checked_at)}</p>
                        )}
                      </div>
                    </li>
                  );
              ...
              <div className="mt-4 flex items-center justify-between gap-3 border-t border-border pt-3">
                {g.go_no_go_decision ? (
                  ...
                  <p className="text-xs text-muted-foreground">
                    {ready ? "All criteria complete — make a Go/No-Go call." : `${CRITERIA.length - doneCount} criteria outstanding.`}
                  </p>
                )}
